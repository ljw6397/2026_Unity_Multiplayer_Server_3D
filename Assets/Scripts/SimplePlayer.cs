...
public class SimplePlayer : NetworkBehaviour
{
    ...
    [SerializeField] private float cameraSensivity = 3.0f;
    ...
    [Networked] private NetworkObject HoldBox { get; set; }
    ...
    private void LateUpdate()
    {
        ...
        cameraYaw += mouseX * cameraSensivity;
        cameraPitch -= mouseY * cameraSensivity;
    ...
    private bool TryDropHoldBox()
    {
        if (!Object.HasInputAuthority) return false;

        if (HoldBox == null) return false;

        PickableBox box = HoldBox.GetComponent<PickableBox>();
